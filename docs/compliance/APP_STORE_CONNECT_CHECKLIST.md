# App Store Connect checklist (US only) — challenge / press-your-luck framing

**Product positioning:** Screen-time helper with optional **challenges** (skill + press-your-luck pots). No currency, no purchases for minutes, no cash-out. Individual Developer account. Declare **Simulated Gambling = None**.
...
- Primary: **Productivity** (or **Lifestyle**) — match Second Thought precedent
...
• Optional challenges: Math Sprint (skill), Safe tiles, Timing run, Color spin, Blackjack, and more
...
No real money. No deposits. No cash-out. Not a casino.
...
**What’s New (resubmit):**  
Challenges now use press-your-luck pots you can bank anytime, plus Math Sprint skill cover. Optional commit defaults to off. Screen-time helper positioning unchanged.

## Review notes (paste)

```
Screel is a screen-time / digital-wellbeing helper (same category as challenge-based app blockers).
...
• Press-your-luck challenges build a temporary BONUS POT. Users may bank the pot anytime. Continuing can wipe the unbanked pot. This is foregone bonus time, not a wager against a purchased currency.
...
Store screenshots should lead with the bank / Screen Time setup and Math Sprint.
```

## Reply to prior rejection (paste in Resolution Center)

```
Thank you for the feedback. Screel is a screen-time helper, not a casino.

Minigames are framed as challenges (including a Math Sprint skill challenge). They build a temporary bonus pot the user can bank anytime; missing wipes only the unbanked pot. An optional commit (default off) can reduce today’s allowance as intentional focus friction — never purchased currency, never cash-out.

We declare Simulated Gambling = None, refresh metadata to match, and resubmit a new binary. Please re-review as a screen-time / digital-wellbeing app suitable for an Individual developer account.
```
...
- [ ] New binary with challenge / pot model
- [ ] Age ratings: Simulated Gambling = None
- [ ] Listing copy matches above (screenshots lead with bank + Math Sprint)
